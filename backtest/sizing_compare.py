...
async def build_candidates(symbols, bars):
    ...
    for i, date in enumerate(dates):
        ...
        by = {}
        for sg in msig:
            by.setdefault(sg.symbol, []).append((W_MOM, sg))
        for sg in rsig:
            by.setdefault(sg.symbol, []).append((W_MR, sg))
        for sym, lst in by.items():
            acts = {sg.action.value for _, sg in lst}
            ...
            if score >= MIN_SCORE:
                ...
                if atr > 0 and price > 0:
                    cands_by_date[date].append(
                        {"sym": sym, "score": score, "atr": atr, "rel_vol": atr / price}
                    )
    ...
    for i, date in enumerate(dates):
        ...
        for cnd in pending:
            ...
            if b is None:
                continue
            equity = cash + sum(
                p["qty"] * (bar(s, date)["close"] if bar(s, date) is not None else p["entry"])
...
async def main():
    ...
    print(
        f"Kandidaten-Signale: {sum(len(v) for v in cands.values())} | Median ATR/Kurs {median_rv:.4f}\n"
    )
    ...
    print(
        f"{'Modell':26} {'Return':>8} {'Sharpe':>7} {'MaxDD':>7} {'PF':>6} {'Win':>6} {'#Tr':>5} {'ØGröße':>7} {'MaxGröße':>8}"
    )
    print("-" * 92)
    for r in results:
        pf = f"{r['pf']:.2f}" if r["pf"] != float("inf") else "inf"
        print(
            f"{r['name']:26} {r['ret']:>7.1f}% {r['sharpe']:>7.2f} {r['dd']:>6.1f}% {pf:>6} {r['wr']:>5.0f}% {r['n']:>5} {r['avg_pct']:>6.1f}% {r['max_pct']:>7.1f}%"
        )
