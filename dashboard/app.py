...
async def whales(request: Request):
    ...
    cursor = await db._db.execute(
        "SELECT * FROM whale_holdings ORDER BY fetched_at DESC LIMIT 200"
    )
    rows = await cursor.fetchall()
    holdings = [dict(r) for r in rows]
    return templates.TemplateResponse(request, "whales.html", {
        "holdings": holdings,
    })
...
async def costs(request: Request):
    cursor = await db._db.execute(
        "SELECT * FROM api_costs ORDER BY timestamp DESC LIMIT 100"
    )
    rows = await cursor.fetchall()
